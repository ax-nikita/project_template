...
class AxImage {
  ...
  function loadImage() {
    ...
    preg_match('/\.(' . implode('|', self::$imgTypes) . ')$/ui', $name, $result);
    if (isset($result[0])) {
      $imageType = mb_substr($result[0], 1);
      preg_match('/[\w\.()]+\./u', $name, $result);
      $clearName = mb_substr($result[0], 0, -1);
  ...
  function compressImage($maxSize = 1920) {
    ...
    if ($max > $maxSize) {
      ...
      $this->image = imagescale($this->image, $newWidth, $newheight, IMG_BICUBIC);
    }
  ...
  function imageMask($mask) {
    if (is_string($mask)) {
      $mask = new AxImage($mask);
    } else if (get_class($mask) == 'GdImage') {
      $image = $mask;
      $mask = new AxImage();
      $mask->image = $image;
    }
    ...
    $mask->goByPixels([
      function ($img, $x, $y) {
        $colorIndex = imagecolorat($img->image, $posX, $posY);
        $color = imagecolorsforindex($img->image, $colorIndex);

        $colorSumm = $color['red'] + $color['green'] + $color['blue'];
        if ($color['alpha'] != 127 && $colorSumm < $mask->maxDark) {
